...
async def whitelist_remove(message: aiogram.types.Message):
    ...
    for i in member_in_db:
        ...
        if whitelist_chat_id == db_chat_id:
            sql.execute(
                f'delete from whitelist where member_id={whitelist_user_id} and chat_id={whitelist_chat_id}')
            sql.commit()

            await bot.send_message(message.chat.id, 'The operation ended successfully')
            return
    await bot.send_message(message.chat.id, 'This member is not in whitelist of this group')
